import { Feature, Point, LineString, Polygon, MultiPolygon, FeatureCollection } from "geojson";
...
/**
   ...
   export class GeoExtensions {
     ...
     */
    static convertRDFeaturesToWGS84(rd: Feature[]) {
        if (!rd || rd.length <= 0) {
            return;
        }
        rd.forEach((f: Feature) => {
            switch (f.geometry.type.toLowerCase()) {
                case 'point':
                    GeoExtensions.convertGeoJsonRDToWGS84((<Feature<Point>>f).geometry.coordinates);
                    break;
                case 'linestring':
                case 'polygon':
                (<Feature<Polygon>>f).geometry.coordinates.forEach((line) => {
                    ...
                    break;
                case 'multipolygon':
                (<Feature<MultiPolygon>>f).geometry.coordinates.forEach((poly) => {
                        poly.forEach((line) => {
                            line.forEach((segment) => {
                                GeoExtensions.convertGeoJsonRDToWGS84(segment);
                            });
    ...
    */
     ...
     */
    static createPolygonFeature(coordinates: Array<Array<Array<number>>>, properties: Object): Feature<Polygon> {
    ...
    static featureInsideBoundingBox(feature : Feature, bboxarray : number[][]) : boolean
    {
